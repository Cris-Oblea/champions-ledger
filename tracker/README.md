...
- `tracker/src/` - **the app. Edit these.** `01-data.js` .. `13-boot.js`,
  ...
  **The entry imports `_legacy.js` FIRST, and that is load-bearing.** A
  ...
  by construction: the bridge imports every converted module at its top, in
  numeric order, so they all run before it does - the order the parts had when
  they were concatenated. `check_order()` asserts it on every build, and says
  so in one line. This is not a bundler quirk; it is what the language does

...
- `tracker/src/` - **the app. Edit these.** `01-data.js` .. `13-boot.js`,
  ...
  page under jsdom. Shipping module scripts would not have failed those tests,
  it would have made them pass by finding nothing. So the modules are linked at
  build time and the tests run on the exact bytes that deploy. esbuild is
  pinned in `package-lock.json` and installed by `npm ci` - never `npx`, which
  fetches whatever is newest at the moment it runs.

  The conversion is INCREMENTAL. A part that is not a module yet is
  concatenated exactly as before into a generated `_legacy.js`, which imports
  what the converted modules export and exports what they still need back. It
  loses a part every time one is converted, and when it is empty the pass is
  done. `_legacy.js` and `_entry.js` are generated - do not edit either, and
  neither is committed.

  **The entry imports `_legacy.js` FIRST, and that is load-bearing.** A
  module's body runs when the import graph reaches it, depth first, in the
  order the imports are written. Import a converted part before the bridge and
  the walk reaches the bridge through it, so the BRIDGE's body runs first - and
  the bridge ends in `13-boot`, which starts the app. It called `buildTabs()`
  before `04-nav` had built `TABS`, and sixteen browser tests went red at once
  with a message that named neither file. Entering through the bridge fixes it
  by construction: the bridge imports every converted module at its top, so
  every one of them runs before it does. `check_order()` asserts exactly that
  on every build and prints the order it found. This is not a bundler quirk; it
  is what the language does with a cycle, and native modules would do the same.

  Among the modules themselves the order is whatever the imports say, and that
  is the improvement over concatenation: `09-gts` runs before `05-box` because
  the box row asks the GTS view for its badges, not because of a number in a
  filename. Nothing relies on the old order any more - the only top-level
  statements left in the converted parts attach handlers to their own
  elements.

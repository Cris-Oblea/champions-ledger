...
prefixes give, into `tracker/index.template.html`'s shell and then into
`tracker/index.html`. **Edit the part, never `index.template.html` (a 23-line
shell of markers) and never `index.html` (generated).** The split is pure
concatenation: a line in a part is that same line in the page, so a stack trace
still points at real code, and the order is load-bearing - `13-boot.js` wires
up a DOM the markup has to have produced first.

`node scripts/check_app.js` reads the parts back as one program, which is the
check the split needs: a `var` two parts each declare is invisible in either
file alone. It runs inside the gate.
